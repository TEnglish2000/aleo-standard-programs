...
for validator_index in "${validator_indices[@]}"; do
  ...
  if [ "$validator_index" -eq 0 ]; then
    tmux send-keys -t "devnet:window$validator_index" "RUST_BACKTRACE=full snarkos start --nodisplay --network $network_id --dev $validator_index --allow-external-peers --dev-num-validators $total_validators --validator --logfile $log_file --rest-rps 10000 --metrics" C-m
  else
    ...
    tmux send-keys -t "devnet:window$validator_index" "RUST_BACKTRACE=full snarkos start --nodisplay --network $network_id --dev $validator_index --allow-external-peers --dev-num-validators $total_validators --validator --logfile $log_file --rest-rps 10000" C-m
  fi
done
